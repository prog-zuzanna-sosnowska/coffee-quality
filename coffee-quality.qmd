...
Powyższy wykres pokazuje dominację niektórych krajów w zestawieniu z innymi. Zauważamy, że najwięcej próbek pochodzi z Meksyku, Kolumbii i Gwatemali. To skłania do pytania, czy wyższa produkcja kawy w tych krajach wynika z jej lepszej jakości. Ta kwestia również zostanie szczegółowo sprawdzona w dalszej części (*rozdział 4.3.*).
...
Na tym etapie analizy możemy zauważyć, że metoda przetwarzania, którą wcześniej uznaliśmy za najpopularniejszą, nie wyróżnia się znacząco w porównaniu do innych metod, zarówno w kontekście zmiennej "Flavor" (podpunkt *a*), jak i "Aroma" (podpunkt *b*). W obu przypadkach średnie wartości dla poszczególnych metod są zbliżone, co sugeruje, że nie ma wyraźnych różnic w tych zmiennych pomiędzy metodami.
...
```{r, echo = FALSE, results='hide'}
data_filtered <- subset(data_clean, Processing.Method %in% c('Washed / Wet', 'Natural / Dry'))

t.test(Total.Cup.Points ~ Processing.Method, data = data_filtered)

```

  **4.3.2. Wpływ zmiennej `Country.of.Origin`**
  
Celem tej analizy jest określenie, który kraj produkuje najlepszą kawę. Ze względu na dużą liczbę krajów w naszym zbiorze danych, aby uniknąć zbyt szczegółowej i czasochłonnej analizy, na początku wykluczymy kraje, które mają mniej niż 30 obserwacji. Takie kraje mogą wprowadzać nieprecyzyjne wnioski. Ponadto, skupimy się na krajach, których średnia punktacja Total.Cup.Point wynosi co najmniej 82 punkty, ponieważ międzynarodowe standardy uznają kawy oceniane na minimalnie 80 punktów i wyżej za kawy najwyższej jakości.


```{r, echo=FALSE}
filtered_countries <- data_clean %>%
...
filtered_data <- data_clean %>%
  filter(Country.of.Origin %in% filtered_countries)
